        ...
        const fetchTeamUsers = async (team) => {
            try {
                ...
                userDetails.forEach(user => {
                    for (let i in user.perfs) {
                        if (!(i in ratings)) ratings[i] = [];
                        ratings[i].push([user.username, user.perfs[i].rating, user.perfs[i].games, user.perfs[i].prov, user.title, user.flair]);
                    }
                });
                ...
                console.error("Error during fetch process:", err.message);
